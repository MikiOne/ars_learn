...
use crate::common::db_mg::PgConn;
use crate::user::user_models::{NewUser, SimpleUser, User};
use diesel::{QueryDsl, SelectableHelper};
use diesel_async::RunQueryDsl;
use log::error;

pub async fn get_by_id(user_id: i32, conn: &mut PgConn) -> BizResult<User> {
    ...
    if user_id != 1 {
        ...
        wa_users.find(user_id).first::<User>(conn).await.map_err(|err| {
...
pub async fn save_user(new_user: NewUser, conn: &mut PgConn) -> BizResult<SimpleUser> {
    use crate::schemas::wa_users::dsl::*;
